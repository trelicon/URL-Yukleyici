...
async def broadcast_handler(m: Message):
    ...
    out = await m.reply_text(
        text=f"Yayın başladı!\nTüm kullanıcılar bilgilendirildiğinde günlük dosyası ile bilgilendirileceksiniz."
    )
    ...
    if failed == 0:
        await m.reply_text(
            text=f"Yayın `{completed_in}` içinde tamamlandı.\n\n"
                 ...
                 f"Toplam yapılan yayın {done}, {success} başarılı ve {failed} başarısız.",
            quote=True
        )
    else:
